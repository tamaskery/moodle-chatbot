...
/**
 * Manager index status endpoint.
 */
final class get_index_status extends external_api {
    /**
    ...
    /**
     * Return the current course index status.
     *
    ...
    public static function execute(int $courseid): array {
        ...
        $config = \block_courseaiguide\local\config\course_config::get((int) $course->id);
        return [
            'status' => $config ? (string) $config->indexstatus : 'disabled',
            'timeindexed' => $config && $config->timeindexed ? (int) $config->timeindexed : 0,
        ];
    }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'status' => new external_value(PARAM_ALPHA, 'Index state'),
            'timeindexed' => new external_value(PARAM_INT, 'Last successful index time or zero'),
        ]);
    }
}
